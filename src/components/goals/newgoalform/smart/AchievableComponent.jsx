...
const AchievableComponent = ({ achievable, setAchievable }) => {
  ...
  return (
    <div>
      ...
      ></textarea>
      <label
        htmlFor="achievableRealistic"
        className="block text-sm font-medium text-gray-700"
      >
        How realistic is the goal based on other constraints?
      </label>
      <textarea
        id="achievableRealistic"
        value={achievable.realistic}
        ...
        {quantifiers.map((quantifier, index) => (
          ...
          >
            ...
            <div className="flex items-center w-full">
              <input
                type="text"
                value={quantifier.label}
                onChange={(e) =>
                  handleQuantifierChange(index, "label", e.target.value)
                }
                placeholder="Metric Label"
                className="mt-1 block w-4/5 border border-gray-300 rounded-md shadow-sm p-2 focus:border-black focus:ring-2 focus:ring-black focus:outline-none"
              />
              <input
                type="number"
                value={quantifier.value}
                onChange={(e) =>
                  handleQuantifierChange(index, "value", e.target.value)
                }
                placeholder="Value"
                className="mt-1 block w-1/5 border border-gray-300 rounded-md shadow-sm p-2 focus:border-black focus:ring-2 focus:ring-black focus:outline-none ml-2"
              />
            </div>
          </div>
        ))}
